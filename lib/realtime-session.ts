...
type StreamEvent =
  ...
  | { type: "text-message"; content?: string; timestamp?: number }
  | { type: "assistant-message"; id?: string; content?: string; toolCalls?: any[] }
  ...
  | { type: "task-complete"; message?: string }
  | { type: "loop-limit-reached"; message?: string }
  | { type: "finish" }
...
export class RealtimeSession {
  ...
  private processLine(rawLine: string) {
    ...
    if (event.type === "text-message") {
      const textMessageEvent = event as { type: "text-message"; content?: string; timestamp?: number };
      if (!textMessageEvent.content) return;
      // Dodaj pełną wiadomość tekstową od AI
      const assistantMessage: Message = {
        id: `assistant-text-${textMessageEvent.timestamp || Date.now()}-${Math.random()}`,
        role: "assistant",
    ...
    if (event.type === "assistant-message") {
      const assistantEvent = event as { type: "assistant-message"; id?: string; content?: string; toolCalls?: any[] };
      if (!assistantEvent.content && !assistantEvent.toolCalls?.length) return;
      // Pełna wiadomość od AI z potencjalnymi tool calls
      const assistantMessage: Message = {
        id: assistantEvent.id || `assistant-${Date.now()}-${Math.random()}`,
        role: "assistant",
    ...
    if (event.type === "task-complete" || event.type === "loop-limit-reached") {
      this.currentTextId = null;
      this.updateSnapshot({ status: "ready" });
      return;
    }
